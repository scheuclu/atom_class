...
for i in 0 1 2  ; do
  for dampval in ${dampvals[@]}; do
    for quantity in PotEng Press Temp KinEng TotEng; do
      ${LOG2TXT} ./logfiles/log.argonBer_dt${timesteps[i]}_damp${dampval}\
               results/Ber_covergence_${quantity}_dt${timesteps[i]}_damp${dampval}.txt\
               Time ${quantity}


      ../plotsimple.py results/Ber_covergence_${quantity}_dt${timesteps[i]}_damp${dampval}.txt\
                       plots/Ber/covergence_${quantity}_dt${timesteps[i]}_damp${dampval}.png\
                       Time ${quantity} Berendsen_${quantity}_over_time_dt${timesteps[i]}_damp${dampval}
    done
  done
done
