...
#done
    ...
    for quantity in Volume PotEng Press Temp KinEng TotEng; do
      ${LOG2TXT} logfiles/log.argonNPT\
                 results/NPT_convergence_${quantity}.txt\
                 Time ${quantity}


      ../plotsimple.py results/NPT_convergence_${quantity}.txt\
                       plots/NPT/convergence_${quantity}.png\
                       Time ${quantity}\
                       NPT_${quantity}_over_time
    done
